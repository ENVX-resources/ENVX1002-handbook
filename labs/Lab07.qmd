...
A study measures dogs' pain using the Glasgow CMPS-SF scale before and after medication. Note that this is ordinal data i.e. pain scale from 0 - 24
...
pain_long <- pivot_longer(pain, cols = c(before, after), 
                          names_to = "time", values_to = "score") ## Pivot longer is a nice command to get data into tidy format for use in ggplot
...
2.  The differences between the paired observations are continuous/ordinal and are reasonably symmetric about the median (looking at the boxplots, this is the case). Note that these tests are reasonably robust to some outliers.
...
Given that the p-value is significant (P<0.05) and looking at the output of the boxplot, we can conclude that the pain scores are significantly lower after treatment. It worked!!
