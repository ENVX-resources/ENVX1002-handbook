...
| Hypothesis testing and estimating adjusted R^2^ are straightforward | Hypothesis testing and estimating R^2^ is a bit more difficult – note the absence of R^2^ in R output |
...
with three models fitted in separate panels; a linear model, quadratic
...
old_par <- par(mfrow = c(3, 1), mar = c(4, 4, 1, 1))

plot_model <- function(pred, col = "black") {
  plot(soil$ph, soil$yield, xlab = "pH", ylab = "Yield (t/Ha)")
  lines(ph.new, pred, col = col, lwd = 2)
}

plot_model(pred.quad, col = "red")
plot_model(pred.lin, col = "blue")
plot_model(pred.cub)
...
::: {.column-margin}
Note: no hypothesis testing yet, just how well the model fits the
data (adjusted R^2^).
:::
...
**b)** The adjusted R^2^ assesses how well the model fits the data while
accounting for the number of predictors. The best model based on this
metric is the cubic polynomial (adjusted R^2^ = 0.234), then the
quadratic polynomial (adjusted R^2^ = 0.217) and finally the linear model
(adjusted R^2^ = -0.0091).

It should be noted that adjusted R^2^ does not replace formal hypothesis
testing. For example, is the extra complexity of the cubic polynomial
worth an increase in adjusted R^2^ of 0.018? Formal hypothesis testing
...
a)  Using adjusted R^2^, what is the model fit like in this model?
...
**a)** Our model fit is ok, with adjusted R^2^ of 0.4515.
...
b)  Using adjusted R^2^, what is the fit like for our quadratic model?
    Is it better than our linear model?
...
**b)** Our model fit even better, with adjusted R^2^ of 0.9980.
