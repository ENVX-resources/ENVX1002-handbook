---
title: "ENVX1002 Practical Topic 11 - Multiple Linear Regression"
output:
  pdf_document:
      fig_width: 7
      fig_height: 6
...
```{r LoadData, echo=T}
library(readxl)
Corn <- read_xlsx("ENVX1002_practical_wk11_data_Regression.xlsx", "Corn")
head(Corn)
```
...
**a)** These results tell you that the correlation between CornP and InorgP the highest (r = 0.720)
The correlation between CornP and OrgP (r = 0.212) or between OrgP and InorgP (r = 0.399) is much weaker.
```
...
```{r, echo=T}
mic <- read_xlsx("ENVX1002_practical_wk11_data_Regression.xlsx", "Microbes")
...
Dippers <- read_xlsx("ENVX1002_practical_wk11_data_Regression.xlsx"
                     ,"Dippers")
```
