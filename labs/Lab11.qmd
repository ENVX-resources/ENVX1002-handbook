...
resources:
  - data/ENVX1002_practical_wk11_data_Regression.xlsx
  - data/Housing.csv
...
1) [ENVX1002_practical_wk11_data_Regression.xlsx](data/ENVX1002_practical_wk11_data_Regression.xlsx)
...
This exercise will use data from the [NOAA Fisheries data portal](https://www.webapps.nwfsc.noaa.gov/apex/parrdata/inventory/datasets). The dataset contains the results of microbial study of Puget Sound, an estuary in Seattle, U.S.A.
...
**a)** The P-value for the F-statistic is 1.318e-10, which is smaller than 0.05. We therefore reject the null hypothesis and can say our model is indeed significant as at least one of our predictors is significant.
...
**b)** Yes; as the P-value for NO3 is 0.00268 (P < 0.05), we can reject the null hypothesis that the true partial regression slope for this variable is equal to zero.

Water_temp and carbon_L_h are not significant predictors (P > 0.05), i.e. we cannot reject the null hypothesis that the true partial regression slope of each is equal to zero.
...
The fit of this model is moderate, with a residual standard error of 250500 and an r^2^-adj = 0.5913.
...
Furthermore this fit was not substantially improved by the removal of the carbon_L_h predictor within the model (r^2^-adj = 0.587).
...
**b)** Scientific conclusion: 
  ...
  Nitrate is a significant predictor of total bacteria count in these Puget Sound water samples (P < 0.05), whereas water temperature and carbon production rate are not significant in this model (P > 0.05). The model accounts for 59.1% of variation in the bacteria counts measured in water samples collected from Puget Sound.
...
**f)** Unlike last week, the P-value for LogCadd is now greater than 0.05 (P = 0.16674), and so we fail to reject the null hypothesis that the partial regression slope is equal to zero.  We can therefore say LogCadd is not a significant predictor of Dipper breeding pair density.
...
As the P-value for the F-test is less than 0.05, we can reject the null hypothesis that all partial regression coefficients are equal to zero. Furthermore, LogStone was the only significant predictor (P < 0.05) and therefore we can only reject the null for this predictor.
...
> Our t-test upon each of our partial regression coefficients supports this, as square footage and occupant number both have p-values below 0.05.
