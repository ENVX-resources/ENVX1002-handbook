...
str(housing)
```
Looking at the correlation matrix and teh plots, it looks like bedroom number has a very weak relationship to Price, and it is fairly correlated to room number. I will leave it out of the model to try to keep all the variables independent from each other. Everything ele will go in the model though.
...
par(mfrow = c(1,1))
```
> Assumptions are beautiful and prefect. Relaationship is linear, data is normally distributed, there is no fanning in the scale-location plot, and the residuals vs leverage doesn't even show the Cook's distance line.
...
```
> Not too much correlation between independent variables, but appliances used has a very weak relationship with energy consumption. I'm going to leave it out of my model.
...
par(mfrow = c(1,1))
```
> All the assumptions look fine. The points are starting to veer off the line at the edges of the qqplot, but I'm not worried about it.
...
round(cor(sales),3)
```
> The first variable is just the row id, so I'm going to ignore it. All the independent variables look like they have a relationship wiht the target variable, though the relationship with newspaper ad budget is rather weak. There aren't any particularly strong correlations between the budgets, so I will include all of them in my model.
...
par(mfrow = c(1,1))
```
> All the assumptions look fine. Teh residualsvs fitted plot is a bit smooshed, but the line is relatively straight, so I'm not worried about it.
...
```
    ...
    b)   Are the predictors significant? the tv and radio ad budgets are both significant, but the neewspaper ad budget was not significant (p = 0.86)
...
> Our t-test upon each of our partial regression coefficients supports this, as tv ad budget and radio ad budget both have p values below 0.05. However, newspaper ad budget was not a significant predictor, with p = 0.86.

> The fit of the model is excellent, with ~90% of the variation explained by our model. Since the newspaper ad budget was not significant, we could try running the model again without this variable, to see if it imporves the fit.
