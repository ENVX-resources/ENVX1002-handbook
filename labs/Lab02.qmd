...
Square brackets allow more flexible subsetting using the format: `dataframe[rows, columns]`. Try the following examples:
...
Try these subsetting exercises

1. **Basic subsetting**: Use square brackets to:
   - Extract soil samples at 0cm depth that have clay content greater than 40%
   ```{r}
   #| eval: false
   soil[soil$clay0 > 40, ]
   ```
   - Question: can you extract samples where clay content at 10cm depth is less than 50%?
  


2. **Multiple conditions**: Find samples where:
   - Clay content at 60cm is greater than 30% AND electrical conductivity at 0cm is less than 0.5
   ```{r}
   #| eval: false
   soil[soil$clay60 > 30 & soil$ec0 < 0.5, ]
   ```
    - Question: can you find samples where clay content at 0cm is less than 20% OR pH at 60cm is greater than 6.5?

## Exercise 5: soil statistics

Let's practice calculating some basic statistics for the soil data:

- [Calculate the mean, median, and mode for clay content at 0cm depth]{style="color: blue"}
- [Calculate the range, IQR, variance, and standard deviation for electrical conductivity at 60cm depth]{style="color: blue"}
- [Report these statistics in your Quarto document using inline R code]{style="color: blue"}