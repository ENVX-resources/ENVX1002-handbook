...
Create your Quarto document and save it as `Lab-06.Qmd` or similar. The following data files are required:

1)  [Barley.csv](data/Barley.csv)
2)  [Plant_growth.csv](data/Plant_growth.csv)
3)  [water_quality.csv](data/water_quality.csv)
...
From the boxplot, we can see that there is some indication that the variances are not equal. We can test this assumption using a F-test, Bartlett's test or Levene's test; here we will just use Bartlett's test (look up the different tests for equal variance and pros and cons of each).

$H_0: \text{Variance is equal across all groups}$
$H_1: \text{Variance is NOT equal across all groups}$
...
Here, we have two samples, and the samples are paired as it is a before-after experiment (the data are NOT independent). So we’d like to conduct a paired *t*-test.
...
### Assumptions
#### Normality
#### Homogeneity of variance (if required)
### Test statistic
### P-value
### Conclusion
```

Note 1. The data is *not* tidy. The code below will convert the data to the long format and assign it to `tidy_plant` which can be useful for ggplot.
Note 2. The data are paired so we look at the difference when testing hypotheses.
Note 3. We cannot perform a paired t.test on data in long format - this is to ensure the data remains "tied/paired" i.e. Floris before heart rate doesn't get mixed up with Liana's after heart rate. 
...
```{r}
#qqplots
plant_growth$Diff <- plant_growth$Treated - plant_growth$Control
ggplot(plant_growth, aes(sample = Diff)) +
  stat_qq() +
  stat_qq_line() 
```

The QQ-plots show that the data are normally distributed, but there are some outliers worth checking and the dataset is small. A formal test for normality will give us a better idea of whether the assumption is met.

```{r}
# perform test on groups
shapiro.test(plant_growth$Diff)
```
...
**Remember** you must indicate paired = TRUE!!

```{r}
t.test(plant_growth$Treated,
  plant_growth$Control,
  paired = TRUE
)
```
...
``` {r}
water_quality$TurbLog10 <- log10(water_quality$Turbidity)
```
...
#Make boxplot
ggplot(water_quality, aes(x = Time, y = Turbidity)) +
  geom_boxplot()
```
...
```{r}
#qqplots
ggplot(water_quality, aes(sample = Turbidity)) +
  stat_qq() +
  stat_qq_line() +
  facet_wrap(~Time)
```
...
```{r}
ggplot(water_quality, aes(sample = TurbLog10)) +
  stat_qq() +
  stat_qq_line() +
  facet_wrap(~Time)
```
...
```{r}
tapply(water_quality$TurbLog10, water_quality$Time, shapiro.test)
```
...
```{r}
bartlett.test(TurbLog10 ~ Time, data = water_quality)
```
...
```{r}
t.test(TurbLog10 ~ Time, data = water_quality, var.equal = TRUE)
```
...
The mean turbidity values before and after the new filtering process was installed are significantly different ($t = -3.7$, $df = 17$, $p = 0.007$). The new filtering process has effectively a 6.77-fold reduction in turbidity. We are 95% confident that the new filtering process will reduce water turbidity by between 2.3 and 20 fold.

Wait! Why 6.77 times? Recall the log law: log10(A) - log10(B) = log10(A/B)
